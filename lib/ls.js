...
function filterFound (root, args) {
  ...
  while (toMark.length) {
    ...
    Object.keys(markDeps).forEach(function (name) {
      var dep = markDeps[name]
      if (dep.peerMissing) return
      dep._parent = markPkg
      for (var ii = 0; ii < args.length; ii++) {
        if (name === args[ii][0] && semver.satisfies(dep.version, args[ii][1], true)) {
          dep._found = 'explicit'
...
function makeArchy_ (data, long, dir, depth, parent, d) {
  ...
  if (data._deduped) out.label += ' deduped: ' + data._deduped
